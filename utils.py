...
def train_one_epoch_rgs(model, optimizer, data_loader, device, epoch):
    '''训练一轮模型'''
    begin_state = model.begin_state(device,data_loader.batch_size)
    loss_list = []
    model.to(device)
    ...
    for step, data in enumerate(data_loader):
        ...
        pred,state = model(images.to(device),begin_state)
...
def train_one_epoch_cls(model, optimizer, data_loader, device, epoch):
    '''训练一轮模型'''
    model.to(device)
